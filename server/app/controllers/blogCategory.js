import modelBlogCategory from "../models/blogCategory.js";
import asynHandler from "express-async-handler";

const createCategory = asynHandler(async (req, res) => {
  if (Object.keys(req.body).length === 0) throw new Error("Missing inputs");
  const newCategory = await modelBlogCategory.create(req.body);
  return res.status(200).json({
    success: newCategory ? true : false,
    newCategory: newCategory ? newCategory : "can't create blog - category",
  });
});

const getCaregories = asynHandler(async (req, res) => {
  const categories = await modelBlogCategory.find().select("_id title");
  return res.status(200).json({
    success: categories ? true : false,
    categories: categories ? categories : "can't get blog - category",
  });
});
const updateCategory = asynHandler(async (req, res) => {
  const { id } = req.params;
  const updateCategory = await modelBlogCategory.findByIdAndUpdate(
    id,
    req.body,
    {
      new: true,
...
const deleteCategory = asynHandler(async (req, res) => {
  const { id } = req.params;
  const deleteCategory = await modelBlogCategory.findByIdAndDelete(id, {
    new: true,
  });
  return res.status(200).json({
